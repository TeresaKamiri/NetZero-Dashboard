...
primary_year = test_k.get("primary_target_year")
primary_label = f"{primary_year} breach risk" if primary_year is not None else "Primary breach risk"
...
c1.metric(
    primary_label,
    f"{test_k['breach_risk_primary']:.1%}" if np.isfinite(test_k["breach_risk_primary"]) else "N/A",
    f"{(base_k['breach_risk_primary']-test_k['breach_risk_primary']):+.1%}" if np.isfinite(test_k["breach_risk_primary"]) and np.isfinite(base_k["breach_risk_primary"]) else "outside horizon",
)
